...
A window of days is defined as contiguous range of days. Thus, there are exactly N−K+1N−K+1 windows where this metric needs to be computed. A non-decreasing subrange is defined as a contiguous range of indices [a,b][a,b], a<ba<b, where each element is at least as large as the previous element. A non-increasing subrange is similarly defined, except each element is at least as large as the next. There are up to K(K−1)/2K(K−1)/2 of these respective subranges within a window, so the metric is bounded by [−K(K−1)/2,K(K−1)/2][−K(K−1)/2,K(K−1)/2].

Constraints
1≤N≤100,0001≤N≤100,000 days
1≤K≤N1≤K≤N days
...
Output Format
Line 1..: N−K+1N−K+1 integers, one integer for each window's result on each line
...
int main()
{
    ...
    cin >> numberOfDays >> windowSize;

    vector<int> upvoteCountData(numberOfDays);

    for (int i=0; i<numberOfDays; ++i)
    {
        cin >> upvoteCountData[i];
    }
